...
void app_main() {
  ...
  uint8_t head_offset_d  = pref.getULong(PREF_HEAD_OFFSET_NAME,0);
  const auto default_cfg   = ::lane::LaneConfig{
        .color         = color_d,
        .line_length   = lane::meter(line_length_d),//common::lanely::meter(line_length)
        .active_length = lane::meter(active_length_d),//lane::meter(active_length)
        .finish_length = lane::meter(total_length),
        .line_LEDs_num = line_LEDs_num_d,//49*10+5
        .fps           = DEFAULT_FPS,
        .finish_time   = finish_time_d,
        .head_offset = head_offset_d,//65
  };
  const uint8_t pace_num_d = pref.getULong(PREF_PACE_NUM_NAME, 1);
  float pace_time_d[5]= {};
  for(uint8_t i = 0; i < pace_num_d; i++) {
    pace_time_d[i]=pref.getFloat(static_cast<String>(i).c_str(),20.0);
  }
  const auto else_d = pref.getFloat(PREF_ELSE_DEAL_NAME,0);
  const auto surface_time_d = pref.getFloat(PREF_SURFACE_TIME_NAME,0);
  const auto surface_range_d = pref.getFloat(PREF_SURFACE_RANGE_NAME,0);
  ...
  const auto default_pace = ::lane::LanePace{
        .pace_num = pace_num_d,
        .pace_time = {pace_time_d[0],pace_time_d[1],pace_time_d[2],pace_time_d[3],pace_time_d[4]},
        .else_deal  = else_d,
        .platform_surface_time = surface_time_d,
        .platform_surface_range = surface_range_d,
        .acceleration = acce_d,
        .turn_time = turn_d ,
        .start_time = 0,
        .pace_mode  =mode_d,
